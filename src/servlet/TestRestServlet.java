...
public class TestRestServlet extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
   ...
   // System.out.println(path);
    try {
    	...
    	Filme fil = new Filme();
       	String filmeOuCategoria =  buscaUrlFilmeCategoria(path, path);
  ...
  protected String buscaUrlFilmeCategoria( String path, String nome) throws ServletException{
	  
	  Categorias cat = new Categorias();
	  Filme fil = new Filme();
	  if(cat.buscaNomeCategoria(path).length()>0) return "categoria"; //categoria
	  if(fil.buscaNomeFilme(path).length()>0) return "filme"; //filme
